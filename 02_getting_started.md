...
- [Getting Started](#cha:Getting-Started)
  ...
  - [Using HDF5 for file I/O](#using-hdf5-for-file-io)
...
2.  You may want to change ADIOS related values in the `setup/constants.h` file. The default values probably suit most cases.
...
In the main `Par_file`, you will then have to turn on the HDF5 flag `HDF5_ENABLED`. Note that additional MPI processes can be launched specifically to handle the file I/O in an asynchronous way. The number of these additional MPI processes is specified by the parameter `HDF5_IO_NODES`, such that the total number of MPI processes to launch the executables becomes `NPROC + HDF5_IO_NODES`.
...
Now that you have set the compiler information, you need to select a number of flags in the `setup/constants.h` file depending on your system:
