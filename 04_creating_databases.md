...
`LTS_MODE`  
Turning this flag on will use a local-time stepping scheme for the Newmark time scheme (Rietmann et al. 2015, 2016). It will re-distribute the mesh partitioning to balance out elements with similar local-time step sizes. This feature can help improve the time-to-solution when the mesh contains elements of various sizes, in particular when the mesh contains only a few very small elements that would limit the global time step size.

&nbsp;

`NGNOD`  
The number of nodes for 2D and 3D shape functions for hexahedra. We use either 8-node mesh elements (bricks) or 27-node elements. If you use the internal mesher, the only option is 8-node bricks (27-node elements are not supported). `CUBIT` does not support HEX27 elements either (it can generate them, but they are flat, i.e. identical to HEX8). To generate HEX27 elements with curvature properly taken into account, you can use Gmsh <http://gmsh.info>
...
`gll`  
For a user-specified isotropic model which uses external binary files for $v_{p}$, $v_{s}$ and $\rho$. Binary files are given in the same format as when outputted by the `xgenerate_databases` executable when using option `SAVE_MESH_FILES`. These binary files define the model parameters on all GLL points which can be used for iterative inversion procedures. Note that for simulation setups with attenuation, it will also read in the external binary mesh files for $Q_{\kappa}$ and $Q_{\mu}$. Note that Qmu is always equal to Qs, but Qkappa is in general not equal to Qp. To convert one to the other see `doc/note_on_Qkappa_versus_Qp.pdf` and in folder `utils/small_utilities/attenuation`, the tool `conversion_from_Qkappa_Qmu_to_Qp_Qs_from_Dahlen_Tromp_959_960.f90`.
...
`SAVE_MESH_FILES`  
Set this flag to `.true.` to save [ParaView](www.paraview.org) mesh files for subsequent viewing. Turning the flag on generates large (distributed) files in the `LOCAL_PATH` directory. See Section [\[sec:Mesh-graphics\]](#sec:Mesh-graphics) for a discussion of mesh viewing features.
...
`HDF5_ENABLED`  
Turn this flag on to enable HDF5 file I/O. If set to `.false.`, subsequent HDF5 parameters will not be considered.

`HDF5_FOR_MOVIES`  
Turn this flag on to use HDF5 format for movie outputs.

`HDF5_IO_NODES`  
Setting this number \>0 will allocate additional MPI processes specifically to handle the HDF5 file I/O in an asynchronous way.
...
Rietmann, Max, M. Grote, D. Peter, and O. Schenk. 2016. “Newmark Local Time Stepping on High-Performance Computing Architectures.” *J. Comput. Phys* 334: 308–26.

Rietmann, Max, D. Peter, O. Schenk, B. Ucar, and M. Grote. 2015. “Load-Balanced Local Time Stepping for Large-Scale Wave Propagation.” In *29th IEEE International Parallel & Distributed Processing Symposium (IPDPS)*, 925–35. Hyderabad, India: IEEE CPS.
