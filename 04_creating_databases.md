...
`LTS_MODE`  
Turning this flag on will use a local-time stepping scheme for the Newmark time scheme. It will re-distribute the mesh partitioning to balance out elements with similar local-time step sizes. This feature can help improve the time-to-solution when the mesh contains elements of various sizes, in particular when the mesh contains only a few very small elements that would limit the global time step size.
